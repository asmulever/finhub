-- Reset completo de la base finhub + esquema principal + snapshots R2Lite
-- ATENCIÓN: DROP DATABASE finhub
...
-- Snapshots R2Lite
CREATE TABLE `r2_price_snapshot` (
    `id` BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `symbol` VARCHAR(32) NOT NULL,
    `category` ENUM('ACCIONES_AR','CEDEAR','BONO','MERCADO_GLOBAL','FX','OTHER') NOT NULL,
    ...
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY `uniq_symbol_provider_asof` (`symbol`, `provider`, `as_of`),
    KEY `idx_symbol_asof` (`symbol`, `as_of`),
