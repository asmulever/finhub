...
/**
 * Servicio Application para CEDEARs desde RAVA con cache y backoff.
 */
final class RavaCedearsService
{
    private RavaCedearsClient $client;
    private RavaCedearsCache $cache;
    private LoggerInterface $logger;
    private \DateTimeZone $tz;
    private int $marketOpenMinutes = 11 * 60; // 11:00 ART aprox.
    private int $marketCloseMinutes = 18 * 60; // 18:00 ART aprox.

    public function __construct(
        RavaCedearsClient $client,
    ...
    /**
     * Devuelve lista normalizada de CEDEARs con metadatos de cache.
     *
     * @return array{items:array<int,array<string,mixed>>,meta:array<string,mixed>}
     */
    public function listCedears(): array
    {
        ...
        try {
            $raw = $this->client->fetchCedearsRaw();
            ...
            $this->logger->info('rava.cedears.fetch_failed', [
            ...
            throw new \RuntimeException('No se pudo obtener CEDEARs desde RAVA', 502);
        }
    }

    private function resolveTtlSeconds(\DateTimeImmutable $now): int
    {
        return $this->isMarketOpen($now) ? 90 : 1800; // 60-120s mercado, 30m fuera
    }
    ...
    private function isMarketOpen(\DateTimeImmutable $now): bool
    {
        ...
        $day = (int) $local->format('N'); // 1=lunes ... 7=domingo
        if ($day >= 6) {
            return false;
        }
    ...
    private function normalizeItem(string $symbol, array $row): array
    {
        ...
        return [
            ...
            'ccl_ultimo' => $this->floatOrNull($row['ccl_ultimo'] ?? null),
            'as_of' => $snapshotAt,
            'source' => 'rava',
        ];
    }
    ...
    private function buildSnapshotAt(?string $date, ?string $time): ?string
    {
        ...
        try {
            ...
            $this->logger->info('rava.cedears.datetime_parse_failed', [
                'value' => $candidate,
