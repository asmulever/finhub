...
/**
 * Servicio Application para Acciones Argentinas desde RAVA (agrupadas por panel).
 */
final class RavaAccionesService
{
    private RavaAccionesClient $client;
    ...
    /**
     * Devuelve lista de acciones (flatten) con segment derivado de panel.
     *
     * @return array{items:array<int,array<string,mixed>>,meta:array<string,mixed>}
     */
    public function listAcciones(): array
    {
        ...
        try {
            $rawPanels = $this->client->fetchAccionesRaw();
            $items = $this->normalizePanels($rawPanels);
            $data = [
                'items' => $items,
                'meta' => [
                    'count' => count($items),
            ...
            $this->logger->info('rava.acciones.fetch_failed', [
            ...
            throw new \RuntimeException('No se pudo obtener Acciones Argentinas desde RAVA', 502);
        }
    }
    ...
    {
        $items = [];
        foreach ($panels as $panelKey => $list) {
            if (!is_array($list)) {
                continue;
            }
            foreach ($list as $row) {
                ...
                $items[] = $this->normalizeItem($symbol, $panelKey, $row);
            }
        }
        return $items;
    }
     ...
     */
    private function normalizeItem(string $symbol, string $panelKey, array $row): array
    {
        ...
        return [
            ...
            'segment' => $this->stringOrNull($panelKey),
    ...
    private function buildSnapshotAt(?string $date, ?string $time): ?string
    {
        ...
        try {
            ...
            $this->logger->info('rava.acciones.datetime_parse_failed', [
                'value' => $candidate,
