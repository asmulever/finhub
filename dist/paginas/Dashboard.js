...
var renderDolares = () => {
  ...
  if (!app)
    return;
  if (state.error) {
    app.innerHTML = createCard("D\xF3lares", `<p>${state.error}</p>`);
    return;
  }
  ...
  app.innerHTML = createCard("D\xF3lares (Rava)", `${fetched}${table}`);
};
var loadDolares = async () => {
  try {
    ...
    state.error = error?.error?.message ?? "No se pudieron cargar las cotizaciones de d\xF3lares";
  }
  renderDolares();
};
